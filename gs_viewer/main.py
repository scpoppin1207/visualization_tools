#!/usr/bin/env python
"""Launch the GS Viewer desktop application."""
...
def check_installation() -> int:
    ...
    sample = APP_DIR / "examples" / "sample_gaussians.ply"
    visualizer = GaussianVisualizer(verbose=False)
    visualizer.load_ply_data(sample)
    xyz, *_ = visualizer.select_for_display(max_gaussians=64)
    if visualizer.xyz is None or len(visualizer.xyz) == 0:
        raise RuntimeError(f"Unable to read bundled sample: {sample}")

    print("GS Viewer environment OK")
    print(f"  Qt: {QtCore.QT_VERSION_STR}")
    print(f"  VTK: {vtk.vtkVersion.GetVTKVersion()}")
    print(f"  Mayavi: {mayavi_version}")
    print(f"  Open3D: {o3d.__version__}")
    print(f"  Sample Gaussians: {len(visualizer.xyz)} (display subset {len(xyz)})")
    return 0


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Interactive 3D Gaussian PLY viewer")
    parser.add_argument(
        "files",
        nargs="*",
