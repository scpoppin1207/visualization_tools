#!/usr/bin/env python
"""CLI entry point for global (top-down) voxel occupancy visualization."""
...
def main():
    if len(sys.argv) != 9:
        print(
            "Usage: python vis_occ_glob.py <pcd_root> <pcd_fold> <pcd_scene> <pcd_name> "
    ...
    visualizer = VoxelVisualizer(view_mode=VoxelVisualizer.VIEW_GLOBAL)
    camera_params = visualizer.resolve_camera_params(pcd_scene, use_zoom=use_zoom)
    image_size = camera_params.get("image_size", [800, 800])
    output_path = VoxelVisualizer.build_output_path(
        pcd_root, output_folder, pcd_scene, pcd_name,
        view_mode=VoxelVisualizer.VIEW_GLOBAL, use_zoom=use_zoom,
    )
